...
class TablePdfApi {
  static Future<pw.Document> generateTablePdf(UserModel user, List<EventModel> events) async {
    ...
    // Сортируем по дате
    events.sort((a, b) => DateTime.parse(a.eventDate).compareTo(DateTime.parse(b.eventDate)));

    // Заголовки таблицы
    final headers = [
      '№', 'Название мероприятия', 'Дата', 'Вид Деятельности*', 'Статус**',
      'Уровень***', 'Документ****', 'Баллы'
    ];
    ...
    final totalPoints = events.fold<double>(0, (sum, e) => sum + e.points);

    // Добавляем итоговую строку
    data.add([
      'Итог', // первая ячейка
      '', '', '', '', '', '', // пустые для остальных колонок кроме последней
      totalPoints.toString(), // последняя колонка с суммой
    ]);
          ...
          pw.Center(
            child: pw.Column(
              children: [
                ...
                pw.Text('в период с 1 февраля 2025 года по 31 января 2026 года', style: pw.TextStyle(fontSize: 12, font: regularFont)),
              ],
            ),
          ),
